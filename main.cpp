...
int main(int argc, char * argv[])
{
    ...
    ("debug,d", po::value<bool>()->default_value(false), "Start in debug mode")
    ("quality,q", po::value<int>()->default_value(90), "JPEG compression level of the image 1-100")
    ("width,w", po::value<int>()->default_value(640), "Width of the frames")
    ("height,h", po::value<int>()->default_value(480), "Height of the frames")
    ("framerate,f", po::value<int>()->default_value(30), "The frame rate")
    ("bufferSize,b", po::value<int>()->default_value(3), "Wait for n frame before sending")
    ("uri,u", po::value<string>()->default_value("tcp://127.0.0.1:5563"), "ZMQ publish URI");
    po::variables_map vm;
    ...
    BOOST_LOG_TRIVIAL(debug) << width << "x" << height << " " << framerate << " fps " << endl;
